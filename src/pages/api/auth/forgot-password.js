...
export default async function handler(req, res) {
  const corsHeaders = setCorsHeaders(res);
  ...
  try {
    ...
    const { resetToken, resetTokenExpiry } = generateResetTokenAndExpiry();
    await updateResetTokenAndExpiry(user.email, resetToken, resetTokenExpiry);
    ...
    console.error(error); // Log the error with an error level
...
function generateResetTokenAndExpiry() {
  const resetToken = crypto.randomBytes(32).toString("hex");
  const resetTokenExpiry = new Date(Date.now() + 3600000); // 1 hour
  return { resetToken, resetTokenExpiry };
}

async function updateResetTokenAndExpiry(email, resetToken, resetTokenExpiry) {
    ...
    html: `
      <div class="max-w-lg mx-auto my-10 bg-white p-8 rounded-lg shadow-md">
        ...
        <div class="text-center">
            <a 
              href="${process.env.DOMAIN}/resetpassword?token=${resetToken}" 
